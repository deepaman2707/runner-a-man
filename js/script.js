...
function startMusic() {
  ...
  musicInterval = setInterval(() => {
    if (!musicOn || paused || gameEnded) return;
...
function clearEntities() {
  [...obstacles, ...coinItems, ...powerups, ...bullets, ...bossBullets].forEach(item => item.el.remove());
  obstacles = [];
  coinItems = [];
  powerups = [];
  bullets = [];
...
function resetGame() {
  ...
  if (ground) ground.style.animationDuration = "1s";
...
function spawnBoss() {
  ...
  setTimeout(() => bossAlert.classList.add("hidden"), 1500);
...
addTap(startBtn, onStartPressed);
addTap(restartBtn, onRestartPressed);
addTap(resumeBtn, onResumePressed);
addTap(pauseBtn, onPausePressed);
addTap(musicToggleBtn, onMusicPressed);
addTap(jumpBtn, onJumpPressed);
addTap(shootBtn, onShootPressed);
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.code === "KeyF") onShootPressed();
  if (e.code === "KeyP") onPausePressed();
  if (e.code === "KeyR" && gameEnded) onRestartPressed();
});
